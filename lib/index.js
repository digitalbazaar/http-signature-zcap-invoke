 ...
 */
import {base64Encode, base64urlEncode, isBrowser} from './util.js';
import {
...
/**
 * @typedef {object} InvocationSigner
 * @property {string} id - Key ID used to verify the HTTP signature.
 * @property {function({data: Uint8Array}): Promise<Uint8Array>} sign - Signs
 *   the provided data bytes and returns a signature.
 */

/**
 * @typedef {object} ZCap
 * @property {string} id - The capability ID.
 * @property {string} [parentCapability] - Present on delegated zcaps; its
 *   presence causes the full object to be gzip-compressed and base64url-encoded
 *   into the `capability-invocation` header rather than using only the ID.
 */

/**
 * @typedef {object} SignedHeaders
 * @property {string} authorization - HTTP Signature `Authorization` header.
 * @property {string} capability-invocation - Zcap invocation header
 *   (`zcap id="…"` for root zcaps; `zcap capability="…"` for delegated).
 * @property {string} [digest] - Body digest header; present when `body` or
 *   `json` was provided.
 * @property {string} [content-type] - Content-Type header; present when
 *   `body` or `json` was provided and not already set by the caller.
 */
 ...
 * @param {Blob|Uint8Array} [options.body] - Request body; not allowed when
 *   when `options.json` is provided.
 * @param {object} [options.json] - An optional json object representing an
 *   HTTP JSON body, if any; not allowed when `options.body` is provided.
 * @param {string|ZCap} options.capability - Either a string to invoke a root
 ...
 *   capability.
 * @param {InvocationSigner} options.invocationSigner - An invocation signer
 *   object that includes an `id` representing the key ID for verification of
 *   the HTTP signature and a `sign` function that takes the `{data}` to sign
 ...
 *
 * @returns {Promise<SignedHeaders>} Resolves to the signed headers object
 *   (the input `headers` with lowercased keys, augmented with
 *   `capability-invocation`, `authorization`, and optionally `digest` and
 *   `content-type`).
 */
export async function signCapabilityInvocation({
  ...
  try {
    ...
    // `body` and `json` are not allowed at the same time
    if(body !== undefined && json !== undefined) {
      throw new Error('"body" and "json" must not be provided together.');
    }
    ...
    // if a body is given, compute its digest if not already given
    const hasBody = !(body === undefined && json === undefined);
    if(hasBody && signed.digest === undefined) {
      signed.digest = await createHeaderValue({
        data: body ?? json, useMultihash: true
      });
      // add content-type for body if not already provided
    ...
    // sign over content-type if present
    if(signed['content-type'] !== undefined) {
      includeHeaders.push('content-type');
    }
    // sign over digest if present
    if(signed.digest !== undefined) {
